...
int main()
{
  ...
  try
  {
    ...
    constexpr size_t numQubits = 10; // Number of qubits
    bool use_single_task = false;
    constexpr int hidden = 171;      // Hidden integer
    ...
    // Apply Z gate based on the hidden number
    for (int i = 0; i < numQubits; ++i)
    {
      if ((hidden & (1 << i)) != 0)
      {
    ...
    // Apply Hadamard gate again to all qubits
    for (int i = 0; i < numQubits; ++i)
    {
      circuit.h(i);
    }
