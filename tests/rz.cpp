...
/**
 * @brief Testing the Rz gate with angles 0,pi/2,pi,-pi/2 for |0> and |1> states. Uncomment bloch_sphere() to draw the first result.
 *
 *
 */
class RzGateTest : public ::testing::Test
{
protected:
    /**
...
TEST_F(RzGateTest, TestState0_Angles)
{
    ...
    expectedState = {
        ...
        {Complex<float>(sq, -sq), Complex<float>(0.0f, 0.0f)},     // ((sqrt(2)/2)(1-i),0)
    ...
    for (size_t j = 0; j < angles.size(); j++)
    {
        ...
        rz(queue, stateVector_d, numQubits, 0, angle, use_single_task);
        ...
        // int argc = 1;
        // char* argv[] = { (char*)"test_rz_gate.cpp" };  // Mock argument
...
/**
 * @brief Test the Rz gate on the |1> state.
 *
 * This test ensures that the Rz gate applied to the |1> state with angles results in the expected states
 *
 */
TEST_F(RzGateTest, TestState1_Angles)
{
    std::vector<std::vector<Complex<float>>> expectedState(4, std::vector<Complex<float>>(2));

    expectedState = {
        {Complex<float>(0.0f, 0.0f), Complex<float>(1.0f, 0.0f)}, // (0, 1)
        {Complex<float>(0.0f, 0.0f), Complex<float>(sq, sq)},     // (0, (sqrt(2)/2)(1+i))
