...
/**
 * @brief Test suite for the Z gate operation on quantum states. Uncomment bloch_sphere() to draw the first result.
 *
 * This test class verifies the behavior of the Z gate on quantum states |0> and |1>.
 * Specifically, it checks that the Z gate transforms:
 * - |0> -> |0>
 * - |1> -> -|1>
 *
 ...
 */
class ZGateTest : public ::testing::Test
{
protected:
    /**
...
/**
 * @brief Test the Z gate on the |0> state.
 *
 * This test ensures that the Z gate applied to the |0> state results in the |0> state
 *
 */
TEST_F(ZGateTest, TestState0)
{
    std::vector<Complex<float>> expectedState;
    expectedState = {Complex<float>(0.0f, 0.0f), Complex<float>(0.0f, 0.0f)};
    ...
    z(queue, stateVector_d, numQubits, 0, use_single_task);
...
/**
 * @brief Test the Z gate on the |1> state.
 *
 * This test ensures that the Z gate applied to the |1> state results in the -|1> state
 *
 */
TEST_F(ZGateTest, TestState1)
{
    std::vector<Complex<float>> expectedState;
    expectedState = {Complex<float>(0.0f, 0.0f), Complex<float>(-1.0f, 0.0f)};
    stateVector[0] = Complex<float>(0.0f, 0.0f);
