...
/**
 * @brief Testing the Ry gate with angles 0,pi/2,pi,-pi/2 for |0> and |1> states. Uncomment bloch_sphere() to draw the first result.
 *
 *
 */
class RyGateTest : public ::testing::Test
{
protected:
    /**
 ...
 *
 */
TEST_F(RyGateTest, TestState0_Angles)
{
    ...
    expectedState = {
        ...
        {Complex<float>(sq, 0.0f), Complex<float>(sq, 0.0f)},     // (sqrt(2)/2, sqrt(2)/2)
    ...
    for (size_t j = 0; j < angles.size(); j++)
    {
        ...
        ry(queue, stateVector_d, numQubits, 0, angle, use_single_task);
        ...
        // int argc = 1;
        // char* argv[] = { (char*)"test_ry_gate.cpp" };  // Mock argument
 ...
 *
 */
TEST_F(RyGateTest, TestState1_Angles)
{
    std::vector<std::vector<Complex<float>>> expectedState(4, std::vector<Complex<float>>(2));
