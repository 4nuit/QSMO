#ifndef TEST_CNOT_CPP
#define TEST_CNOT_CPP
...
/**
 * @brief Testing the CNOT gate on the four possible states: |00>, |01>, |10>, and |11>.
 *
 * This test ensures that the CNOT gate gives the expected results for each of these states.
 */
class CNOTGateTest : public ::testing::Test
{
protected:
    /**
    ...
    const size_t numQubits = 2; // Two qubits for the CNOT gate
    const size_t numStates = 1 << numQubits; // 4 states for 2 qubits
};

/**
 * @brief Test the CNOT gate on the |00> state.
 *
 * This test ensures that the CNOT gate applied to the |00> state results in the expected |00> state.
 */
TEST_F(CNOTGateTest, TestState00)
{
    ...
    cnot(queue, stateVector_d, numQubits, 0, 1, use_single_task); // Apply CNOT gate on qubits 0 (control) and 1 (target)
...
/**
 * @brief Test the CNOT gate on the |01> state.
 *
 * This test ensures that the CNOT gate applied to the |01> state results in the expected |01> state.
 */
TEST_F(CNOTGateTest, TestState01)
{
...
/**
 * @brief Test the CNOT gate on the |10> state.
 *
 * This test ensures that the CNOT gate applied to the |10> state results in the expected |11> state.
 */
TEST_F(CNOTGateTest, TestState10)
{
...
/**
 * @brief Test the CNOT gate on the |11> state.
 *
 * This test ensures that the CNOT gate applied to the |11> state results in the expected |10> state.
 */
TEST_F(CNOTGateTest, TestState11)
{
