...
// Uncomment these headers when compiling with freeglut. See the other comments in the tests.
...
/**
 * @brief Test suite for the Hadamard gate (H) operation on quantum states. Uncomment bloch_sphere() to draw the first result.
 * 
 * This test class verifies the behavior of the Hadamard gate on quantum states |0> and |1>.
 * Specifically, it checks that the Hadamard gate transforms:
 * - |0> -> |+>  (sqrt(2)/2, sqrt(2)/2)
 * - |1> -> |-> (sqrt(2)/2, -sqrt(2)/2)
 * 
 * The tests perform state vector initialization, the application of the H gate,
 * and verification of the results using `EXPECT_NEAR`.
 */
class HGateTest : public ::testing::Test
{
protected:
    /**
...
/**
 * @brief Test the Hadamard gate on the |0> state.
 * 
 * This test ensures that the Hadamard gate applied to the |0> state results in the |+> state:
 * (sqrt(2)/2, sqrt(2)/2).
 */
TEST_F(HGateTest, TestState0)
{
    std::vector<Complex<float>> expectedState;
    expectedState = {Complex<float>(sq, 0.0f), Complex<float>(sq, 0.0f)};
    ...
    h(queue, stateVector_d, numQubits, 0, use_single_task);
    ...
    // Uncomment these lines when compiling with freeglut
...
/**
 * @brief Test the Hadamard gate on the |1> state.
 * 
 * This test ensures that the Hadamard gate applied to the |1> state results in the |-> state:
 * (sqrt(2)/2, -sqrt(2)/2).
 */
TEST_F(HGateTest, TestState1)
{
    std::vector<Complex<float>> expectedState;
    expectedState = {Complex<float>(sq, 0.0f), Complex<float>(-sq, 0.0f)};
