 ...
 */
class XGateTest : public ::testing::Test
{
protected:
    /**
...
/**
 * @brief Test the X gate on the |0> state.
 *
 * This test ensures that the X gate applied to the |0> state results in the |1> state
 *
 */
TEST_F(XGateTest, TestState0)
{
    ...
    x(queue, stateVector_d, numQubits, 0, use_single_task);
...
/**
 * @brief Test the X gate on the |1> state.
 *
 * This test ensures that the X gate applied to the |1> state results in the |0> state
 *
 */
TEST_F(XGateTest, TestState1)
{
    std::vector<Complex<float>> expectedState;
    expectedState = {Complex<float>(1.0f, 0.0f), Complex<float>(0.0f, 0.0f)};
